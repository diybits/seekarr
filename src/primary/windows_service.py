"""
Windows Service module for Seekarr.
Allows Seekarr to run as a Windows service.
"""
...
logger = logging.getLogger('SeekarrWindowsService')

class SeekarrService(win32serviceutil.ServiceFramework):
    """Windows Service implementation for Seekarr"""

    _svc_name_ = "Seekarr"
    _svc_display_name_ = "Seekarr Service"
    ...
    def __init__(self, args):
        ...
        self.seekarr_app = None
        self.stop_flag = None
        
    def SvcStop(self):
        """Stop the service"""
        logger.info('Stopping Seekarr service...')
        self.ReportServiceStatus(win32service.SERVICE_STOP_PENDING)
        win32event.SetEvent(self.stop_event)
        self.is_running = False

        # Signal Seekarr to stop properly
        if hasattr(self, 'stop_flag') and self.stop_flag:
            logger.info('Setting stop flag for Seekarr...')
            self.stop_flag.set()
    ...
    def main(self):
        """Main service loop"""
        try:
            logger.info('Starting Seekarr service...')
            ...
            background_thread = threading.Thread(
                target=start_seekarr,
                name="SeekarrBackground",
                daemon=True
            )
            ...
            web_thread = threading.Thread(
                ...
                name="SeekarrWebServer",
                daemon=True
            )
            web_thread.start()

            logger.info('Seekarr service started successfully')
            ...
            while self.is_running:
                ...
                if not background_thread.is_alive() or not web_thread.is_alive():
                    logger.error("Critical: One of the Seekarr threads has died unexpectedly")
            ...
            logger.info('Seekarr service is shutting down...')

            # Set the stop event for Seekarr's background tasks
            if not stop_event.is_set():
                stop_event.set()

            # Wait for threads to finish
            logger.info('Waiting for Seekarr threads to finish...')
            background_thread.join(timeout=30)
            web_thread.join(timeout=10)

            logger.info('Seekarr service shutdown complete')

        except Exception as e:
            logger.exception(f"Critical error in Seekarr service: {e}")
            servicemanager.LogErrorMsg(f"Seekarr service error: {str(e)}")


def install_service():
    """Install Seekarr as a Windows service"""
    ...
    try:
        win32serviceutil.InstallService(
            pythonClassString="src.primary.windows_service.SeekarrService",
            serviceName="Seekarr",
            displayName="Seekarr Service",
            description="Automated media collection management for Arr apps",
            startType=win32service.SERVICE_AUTO_START
        )
        print("Seekarr service installed successfully.")
        return True
    except Exception as e:
        print(f"Error installing Seekarr service: {e}")
        return False


def remove_service():
    """Remove the Seekarr Windows service"""
    if sys.platform != 'win32':
        print("Windows service removal is only available on Windows.")
        return False

    try:
        win32serviceutil.RemoveService("Seekarr")
        print("Seekarr service removed successfully.")
        return True
    except Exception as e:
        print(f"Error removing Seekarr service: {e}")
        return False
